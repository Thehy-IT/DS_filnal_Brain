...
from src.training.models import BaselineCNN, EfficientNetModel, get_model
...
class TestGetModelFactory:
    def test_cnn_factory(self, dummy_input):
        model = get_model(model_name="cnn", num_classes=NUM_CLASSES, pretrained=False)
        model.eval()
        with torch.no_grad():
            out = model(dummy_input)
        assert out.shape == (BATCH, NUM_CLASSES)
